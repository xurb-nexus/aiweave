...
2. `docs/architecture/ai_dev_guide.md` §10.2 —— grep 锚 rule-id 索引（R-PERF-* / R-CACHE-* / `R-LOCALMIRROR-STALE-READY`）；§10.10 尾延迟 / 运行时调优类（R-TAIL-*）；§10.9 部署运行时（R-DEPLOY-GOMAXPROCS / R-RUNTIME-MEMLIMIT）；容量伸缩类（R-CAP-*，决策见 [`design-spec/12`](../design-spec/12_capacity_scaling_design.md)）；§10.12 可观测性类（`R-OBS-CONTROL-LOOP-BLIND` / `R-OBS-LATENCY-COUNTER-ONLY` / `R-OBS-DASH-MISSING`，决策见 [`design-spec/13`](../design-spec/13_observability_design.md)；多为选型 / 人工审查级，未来可拆独立 `observability-review`）
...
## 第 4 步：grep 锚检查（R-PERF-* 7 项 + R-DEPLOY/R-RUNTIME/R-TAIL 尾延迟 6 项 + R-CAP 容量伸缩 2 项 + R-OBS 可观测性 3 项 + SLO/profiling 2 项）
