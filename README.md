...
```
用 design-spec 十二视角做架构决策  ──→  产出技术方案(TRD)
        │
        ▼
...
| 场景 | 触发 | 输入 | AI 任务 |
|------|------|------|---------|
| **A0 — design**（技术方案设计 / 上游） | 需求待定架构 | 业务需求 + design-spec | 过十二视角（`/design-solution`）产出技术方案(TRD) → 喂给 A1 / B1 |
...
`design-spec/` 回答 docs-spec 不回答的问题——**面对需求，怎么"做出"架构决策**，而不是决策定了之后怎么写下来。它是"技术方案(TRD)生成"的参考，按 **十二大决策视角（lens）** 组织（01-07 同步请求路径核心 + 08-12 异步 / 契约 / 多副本 / 时间轴四条边），并内置一套从生产实践沉淀的**高性能基线**：
...
**执行器**：`/design-solution {需求}` —— 过十二视角产出 TRD，再进 A1 forward / B1 sync 落地为 docs/ 与代码。
...
│
├── design-spec/             # 🆕 高性能架构方法论（第三支柱 · 技术方案生成参考 · 编号 00-12）
│   ├── 00_design_overview.md       # 设计方法论总纲（十二视角清单 + 统一 lens 九节骨架 + 双源真相边界 + TRD 产物）
...
用 design-spec 十二视角对需求做架构决策，产出 TRD（简单 CRUD 可跳过，直接进阶段 A）：

```
/design-solution {需求}  →  过 IO / 数据建模 / 并发 / 事务 / 缓存 / 稳定性 / 尾延迟
                            / 异步消息 / API 契约 / 定时协调 / 演进迁移 / 容量伸缩 十二视角  →  技术方案(TRD)
```
